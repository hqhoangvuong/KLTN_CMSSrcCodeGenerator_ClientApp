...
import { BillDeleteUIComponent } from './bill-table-ui-action/bill-delete-ui-dialog/bill-delete-ui-dialog.component';
import { ToastrService } from 'ngx-toastr';
import { HttpClient, HttpHeaders } from '@angular/common/http';
import { BillService, Bill } from '@hqhoangvuong/api-client-803868';

@Component({
  selector: 'app-bill-table-ui',
  templateUrl: './bill-table-ui.component.html',
  styleUrls: ['./bill-table-ui.component.scss'],
})

export class BillTableUIComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(BillService) _service: BillService
  ) {
    this.checkTheme();
  ...
  getPagingApi(pageIndex: number, pageSize: number) {
    this._service
      .apiBillPagingGet(pageIndex, pageSize)
      .subscribe((result) => {
  ...
  getAllApi() {
    this._service.apiBillPagingGet(0, 10).subscribe((result) => {
  ...
  onClickDelete(id: any) {
    const dialogRef = this.dialog.open(BillDeleteUIComponent, {
      data: { idSelected: id },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result.data) {
        this._service.apiBillDelete(id).subscribe(
          (res) => {
