...
import { AccountDeleteUIComponent } from './account-table-ui-action/account-delete-ui-dialog/account-delete-ui-dialog.component';
import { ToastrService } from 'ngx-toastr';
import { HttpClient, HttpHeaders } from '@angular/common/http';
import { AccountService, Account } from '@hqhoangvuong/api-client-803868';

@Component({
  selector: 'app-account-table-ui',
  templateUrl: './account-table-ui.component.html',
  styleUrls: ['./account-table-ui.component.scss'],
})

export class AccountTableUIComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(AccountService) _service: AccountService
  ) {
    this.checkTheme();
  ...
  getPagingApi(pageIndex: number, pageSize: number) {
    this._service
      .apiAccountPagingGet(pageIndex, pageSize)
      .subscribe((result) => {
  ...
  getAllApi() {
    this._service.apiAccountPagingGet(0, 10).subscribe((result) => {
  ...
  onClickDelete(userName: any) {
    const dialogRef = this.dialog.open(AccountDeleteUIComponent, {
      data: { idSelected: userName },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result.data) {
        this._service.apiAccountDelete(userName).subscribe(
          (res) => {
  ...
  onClickView(userName: any) {
    this.nameActionTrigger.emit({ type: 'read', idItem: userName });
  }
  ...
  async applyFilter(event: any) {
    if (this.filterKey === '' ) {
      ...
      this.listItems = this.listItems.filter(item => {
        return ("" + item.userName).trim().toLocaleLowerCase().includes(this.filterKey.toLocaleLowerCase());
      })
    } else{
      this.listItems = this.listItems.filter(item => {
        return ("" + item.userName).trim().toLocaleLowerCase().includes(this.filterKey.toLocaleLowerCase());
