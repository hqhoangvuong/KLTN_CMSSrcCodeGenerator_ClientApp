...
@Component({
  selector: 'app-systemlog-table-ui',
  templateUrl: './systemlog-table-ui.component.html',
  styleUrls: ['./systemlog-table-ui.component.scss'],
})

export class SystemLogTableUIComponent implements OnInit {
  _service: SystemLogService;
  ...
  constructor(
    ...
    @Inject(SystemLogService) _service: SystemLogService
  ) {
    this.checkTheme();
  ...
  getPagingApi(pageIndex: number, pageSize: number) {
    this._service
      .apiSystemLogPagingGet(pageIndex, pageSize)
      .subscribe((result) => {
  ...
  getAllApi() {
    this._service.apiSystemLogPagingGet(0, 10).subscribe((result) => {
  ...
  onClickDelete(id: any) {
    const dialogRef = this.dialog.open(SystemLogDeleteUIComponent, {
      data: { idSelected: id },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result.data) {
        this._service.apiSystemLogDelete(id).subscribe(
          (res) => {
